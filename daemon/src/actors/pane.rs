...
impl Pane {
    ...
    fn new(window_handle: WindowHandle) -> Self {
        ...
        let pty_handle = Pty::spawn(handle.clone()).unwrap();
        Self {
            handle,
            window_handle,
            pty_handle,
...
impl PaneHandle {
    ...
    pub async fn send_user_input(&self, bytes: Bytes) -> Result<()> {
        self.tx.send(PaneEvent::UserInput(bytes)).await.unwrap();
        Ok(())
    }
    pub async fn send_output_from_pty(&self, bytes: Bytes) -> Result<()> {
        self.tx.send(PaneEvent::PtyOutput(bytes)).await.unwrap();
        Ok(())
    }
    ...
    pub async fn pty_died(&self) -> Result<()> {
        self.tx.send(PaneEvent::PtyDied).await.unwrap();
        Ok(())
    }
