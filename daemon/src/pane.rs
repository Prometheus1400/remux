...
impl PaneBuilder {
    ...
    pub fn build(self) -> Result<Pane<Focused>> {
        ...
        let pty = PtyProcessBuilder::new(pty_output_tx)
            .with_exit_callback(|| info!("PtyProcess has exited!"))
            .build()?;
...
pub struct Pane<State> {
    pty: PtyProcesss,
    input_task: Task,
    input_tx: UnboundedSender<Bytes>, // this we use to let others send input to the pane
    output_task: Task,
    output_tx: Sender<Bytes>, // keep this around to construct receivers for 'subscribe'
...
impl Pane<Focused> {
    ...
    fn new(
        pty: PtyProcesss,
        input_task: Task,
        input_tx: UnboundedSender<Bytes>,
        output_task: Task,
        output_tx: Sender<Bytes>,
    ) -> Self {
        Self {
