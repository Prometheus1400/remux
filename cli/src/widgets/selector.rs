...
use crate::{prelude::*, widgets::traits::AlternateScreenWidget};

pub struct SelectorWidget<V> {
    items: Vec<V>,
    title: String,
}

impl<V> SelectorWidget<V> {
    pub fn new<T: Into<String>>(items: Vec<V>, title: T) -> Self {
        Self {
            items,
            title: title.into(),
        }
...
impl<V> AlternateScreenWidget<usize> for SelectorWidget<V>
where
    ...
    async fn run(
        ...
        loop {
            term.draw(|f| {
                ...
                let height = (self.items.len() as u16 + 2).min(size.height / 2); // +2 for padding/border
